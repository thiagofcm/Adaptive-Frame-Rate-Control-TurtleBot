...
# Window 2 — Recorder
# Start BEFORE gazebo_goals so it catches the first /goal_pose
...
# Window 3 — Gazebo goals
tmux new-window -t "$SESSION" -n goals
tmux send-keys -t "$SESSION:goals" \
"cd $PROJECT && source /opt/ros/foxy/setup.bash && source install/setup.bash && ros2 run turtlebot3_drl gazebo_goals" C-m
